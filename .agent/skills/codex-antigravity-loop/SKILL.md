---
name: antigravity-codex-loop
description: Orchestrates a dual-AI engineering loop where Antigravity plans and implements, while Codex validates and reviews, with continuous feedback for optimal code quality
---

# Antigravity-Codex Engineering Loop Skill

## Core Workflow Philosophy
This skill implements a balanced engineering loop:
- **Antigravity**: Architecture, planning, and execution
...
## Phase 1: Planning with Antigravity
...
1. Antigravity implements the code using available tools (Edit, Write, Read, etc.)
...
After every change:
1. Send Antigravity's implementation to Codex for review:
   - Bug detection
   - Performance issues
   - Best practices validation
   - Security vulnerabilities
2. Antigravity analyzes Codex's feedback and decides:
   - Apply fixes immediately if issues are critical
   - Discuss with user if architectural changes needed
   - Document decisions made

## Phase 6: Iterative Improvement
1. After Codex review, Antigravity applies necessary fixes
...
1. Antigravity analyzes the root cause
...
1. Antigravity reviews the error/issue
...
| Implement | Antigravity uses Edit/Write/Read tools | Antigravity implements the validated plan |
| Review code | `echo "review changes" \| codex exec --sandbox read-only` | Codex validates Antigravity's implementation |
| Continue review | `echo "next step" \| codex exec resume --last` | Continue validation session |
| Apply fixes | Antigravity uses Edit/Write tools | Antigravity fixes issues found by Codex |
...
4. When Codex warnings appear, Antigravity evaluates severity and decides next steps

## The Perfect Loop
```
Plan (Antigravity) → Validate Plan (Codex) → Feedback →
Implement (Antigravity) → Review Code (Codex) →
Fix Issues (Antigravity) → Re-validate (Codex) → Repeat until perfect
```

This creates a self-correcting, high-quality engineering system where:
- **Antigravity** handles all code implementation and modifications
